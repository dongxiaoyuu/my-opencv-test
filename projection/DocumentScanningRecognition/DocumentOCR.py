# PROJECT: Document OCR recognition文档扫描与文字识别
# 边缘检测--》轮廓获取--》透视变换
import os
import time
import argparse
...
def image_contour(resize_image, processing_image):
    ...
    for c in contours:
        ...
        # the outer contour is rectangle, so len is 4  矩形4个点
        if len(approx) == 4:
            big_contour = approx
            break

    cv2.drawContours(resize_image, [big_contour], -1, (0, 255, 0), 2)
    show_image('contour image', resize_image) # 绘制绿色的轮廓
...
def perspective_transform(original_image, pts):  # 透视变换，文档的水平矫正
    #  输入四个坐标
